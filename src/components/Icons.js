export const Coffee = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='1.5em'
      height='1.5em'
      viewBox='0 0 24 24'
    >
      <mask id='IconifyId18deaf57c6e69cb2e1'>
        <path
          fill='#fff'
          d='M5 6C5 4 7 6 11.5 6C16 6 19 4 19 6L19 7C19 8.5 17 9 12.5 9C8 9 5 9 5 7L5 6Z'
        />
      </mask>
      <g
        fill='none'
        stroke='currentColor'
        strokeLinecap='round'
        strokeLinejoin='round'
        strokeWidth='2'
      >
        <path
          strokeDasharray='32'
          strokeDashoffset='32'
          d='M7.5 10.5C7.5 10.5 8.33 17.43 8.5 19C8.67 20.57 10 21 11 21L13 21C14.5 21 15.875 19.86 16 19C16.125 18.14 17 7 17 7'
        >
          <animate
            fill='freeze'
            attributeName='stroke-dashoffset'
            dur='0.4s'
            values='32;0'
          />
        </path>
        <path
          strokeDasharray='12'
          strokeDashoffset='12'
          d='M16.5 6C16.5 3.5 14 3 12 3C10 3 9.1 3.43 8 4'
        >
          <animate
            fill='freeze'
            attributeName='stroke-dashoffset'
            begin='0.8s'
            dur='0.2s'
            values='12;24'
          />
        </path>
      </g>
      <rect
        width='16'
        height='5'
        x='20'
        y='4'
        fill='currentColor'
        mask='url(#IconifyId18deaf57c6e69cb2e1)'
      >
        <animate
          fill='freeze'
          attributeName='x'
          begin='0.4s'
          dur='0.4s'
          values='20;4'
        />
      </rect>
    </svg>
  )
}

export const Facebook = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='1.5em'
      height='1.5em'
      viewBox='0 0 24 24'
    >
      <path
        fill='none'
        stroke='currentColor'
        strokeLinecap='round'
        strokeLinejoin='round'
        strokeWidth='2'
        d='M7 10v4h3v7h4v-7h3l1-4h-4V8a1 1 0 0 1 1-1h3V3h-3a5 5 0 0 0-5 5v2z'
      />
    </svg>
  )
}

export const Instagram = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='1.5em'
      height='1.5em'
      viewBox='0 0 256 256'
    >
      <path
        fill='currentColor'
        d='M128 80a48 48 0 1 0 48 48a48.05 48.05 0 0 0-48-48m0 72a24 24 0 1 1 24-24a24 24 0 0 1-24 24m48-132H80a60.07 60.07 0 0 0-60 60v96a60.07 60.07 0 0 0 60 60h96a60.07 60.07 0 0 0 60-60V80a60.07 60.07 0 0 0-60-60m36 156a36 36 0 0 1-36 36H80a36 36 0 0 1-36-36V80a36 36 0 0 1 36-36h96a36 36 0 0 1 36 36ZM196 76a16 16 0 1 1-16-16a16 16 0 0 1 16 16'
      />
    </svg>
  )
}

export const Twitter = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='1em'
      height='1em'
      viewBox='0 0 24 24'
    >
      <path
        fill='currentColor'
        d='M18.205 2.25h3.308l-7.227 8.26l8.502 11.24H16.13l-5.214-6.817L4.95 21.75H1.64l7.73-8.835L1.215 2.25H8.04l4.713 6.231zm-1.161 17.52h1.833L7.045 4.126H5.078z'
      />
    </svg>
  )
}

export const Youtube = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='1.5em'
      height='1.5em'
      viewBox='0 0 256 256'
    >
      <path
        fill='currentColor'
        d='m170.49 117.91l-56-36A12 12 0 0 0 96 92v72a12 12 0 0 0 18.49 10.09l56-36a12 12 0 0 0 0-20.18M120 142v-28l21.81 14Zm118.21-73.5a28.05 28.05 0 0 0-16.93-19.14C186.4 35.91 131.29 36 128 36s-58.4-.09-93.28 13.38a28.05 28.05 0 0 0-16.93 19.14C15.15 78.72 12 97.32 12 128s3.15 49.28 5.79 59.48a28.05 28.05 0 0 0 16.93 19.14C68.21 219.55 120.36 220 127.37 220h1.26c7 0 59.16-.45 92.65-13.38a28.05 28.05 0 0 0 16.93-19.14c2.64-10.2 5.79-28.8 5.79-59.48s-3.15-49.28-5.79-59.48ZM215 181.46a4 4 0 0 1-2.34 2.77C182.78 195.76 132.27 196 128.32 196h-.39c-.53 0-53.64.17-84.56-11.77a4 4 0 0 1-2.37-2.77c-1.88-7.24-5-23.82-5-53.46s3.15-46.22 5-53.46a4 4 0 0 1 2.34-2.77C74.29 59.83 127.39 60 127.92 60h.15c.54 0 53.64-.17 84.56 11.77a4 4 0 0 1 2.37 2.77c1.88 7.24 5 23.82 5 53.46s-3.15 46.22-5 53.46'
      />
    </svg>
  )
}

export const Address = (props) => {
  return (
    <svg
      {...props}
      xmlns='http://www.w3.org/2000/svg'
      width='1em'
      height='1em'
      viewBox='0 0 20 20'
    >
      <path
        fill='currentColor'
        d='M14.95 13.955a7.005 7.005 0 0 0 0-9.904a6.998 6.998 0 0 0-9.9 0a7.005 7.005 0 0 0 0 9.904l1.521 1.499l2.043 1.985l.133.118c.775.628 1.91.588 2.64-.118l2.435-2.37zM10 12a3 3 0 1 1 0-6a3 3 0 0 1 0 6'
      />
    </svg>
  )
}

export const Phone = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='1em'
      height='1em'
      viewBox='0 0 24 24'
    >
      <path
        fill='currentColor'
        d='m7.772 2.439l1.077-.344c1.008-.322 2.086.199 2.518 1.217l.86 2.028c.375.883.167 1.922-.514 2.568L9.82 9.706c.117 1.076.478 2.135 1.084 3.177a8.68 8.68 0 0 0 2.271 2.595l2.276-.76c.862-.287 1.801.044 2.33.821l1.232 1.81c.616.904.505 2.15-.258 2.916l-.818.821c-.814.817-1.976 1.114-3.052.778c-2.539-.792-4.873-3.143-7.003-7.053c-2.133-3.916-2.885-7.24-2.258-9.968c.264-1.148 1.082-2.063 2.15-2.404'
      />
    </svg>
  )
}

export const SunIcon = () => {
  return (
    <svg
      className='swap-off fill-current w-10 h-10'
      xmlns='http://www.w3.org/2000/svg'
      viewBox='0 0 24 24'
    >
      <path d='M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z' />
    </svg>
  )
}

export const MoonIcon = () => {
  return (
    <svg
      className='swap-on fill-current w-10 h-10'
      xmlns='http://www.w3.org/2000/svg'
      viewBox='0 0 24 24'
    >
      <path d='M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z' />
    </svg>
  )
}

export const Menu = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='1.5em'
      height='1.5em'
      viewBox='0 0 24 24'
    >
      <path fill='currentColor' d='M3 4h18v2H3zm6 7h12v2H9zm-6 7h18v2H3z' />
    </svg>
  )
}

export const IEEAlogo = () => {
  return (
    <svg
      width='60'
      height='50'
      viewBox='0 0 477 437'
      fill='none'
      xmlns='http://www.w3.org/2000/svg'
    >
      <path
        d='M201.09 238.52C201.029 264.004 208.016 289.009 221.28 310.77C248.706 290.047 267.462 259.88 273.914 226.116C280.365 192.352 274.053 157.395 256.2 128.02C239.01 140.843 225.069 157.521 215.497 176.713C205.926 195.904 200.991 217.074 201.09 238.52Z'
        stroke='#B71375'
        strokeWidth='30'
        strokeMiterlimit='10'
      />
      <path
        d='M123.8 62.6104C59.5 71.8104 10 128.61 10 197.31C10 272.42 69.17 333.31 142.17 333.31'
        stroke='#8B1874'
        strokeWidth='30'
        strokeMiterlimit='10'
      />
      <path
        d='M242.59 104C199.42 37.92 123.42 14 123.42 14L123.8 62.61C55.8 72.19 10.14 128.31 10 193.27'
        stroke='#8B1874'
        strokeWidth='30'
        strokeMiterlimit='10'
      />
      <path
        d='M27.48 264.95C58.59 321.95 127.05 348.48 191.41 324.45'
        stroke='#8B1874'
        strokeWidth='30'
        strokeMiterlimit='10'
      />
      <path
        d='M350.21 376.51C414.51 367.32 466.5 310.51 466.5 241.81C466.5 166.7 407.33 105.81 334.34 105.81'
        stroke='#FC4F00'
        strokeWidth='30'
        strokeMiterlimit='10'
      />
      <path
        d='M233.92 335.12C277.08 401.2 350.21 422.95 350.21 422.95L350.81 376.34C418.81 366.76 466.36 310.81 466.5 245.86'
        stroke='#FC4F00'
        strokeWidth='30'
        strokeMiterlimit='10'
      />
      <path
        d='M449.02 174.17C417.91 117.15 349.42 90.6698 285.1 114.67'
        stroke='#FC4F00'
        strokeWidth='30'
        strokeMiterlimit='10'
      />
    </svg>
  )
}

export const Play = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='1em'
      height='1em'
      viewBox='0 0 24 24'
    >
      <g fill='#B71375' fillRule='evenodd' clipRule='evenodd'>
        <path d='M11.075 7.508c-1.329-.784-2.825.283-2.825 1.705v5.574c0 1.422 1.496 2.489 2.825 1.705l4.72-2.787c1.273-.752 1.273-2.658 0-3.41zM9.75 9.213c0-.198.096-.337.21-.408a.323.323 0 0 1 .352-.005l4.72 2.787a.465.465 0 0 1 .218.413a.465.465 0 0 1-.218.413l-4.72 2.787a.323.323 0 0 1-.353-.005a.467.467 0 0 1-.209-.408z' />
        <path d='M12 1.25C6.063 1.25 1.25 6.063 1.25 12S6.063 22.75 12 22.75S22.75 17.937 22.75 12S17.937 1.25 12 1.25M2.75 12a9.25 9.25 0 1 1 18.5 0a9.25 9.25 0 0 1-18.5 0' />
      </g>
    </svg>
  )
}

export const PlayAudio = (props) => {
  return (
    <svg
      {...props}
      xmlns='http://www.w3.org/2000/svg'
      width='1.5em'
      height='1.5em'
      viewBox='0 0 16 16'
    >
      <g fill='none'>
        <g clipPath='url(#IconifyId18e1439b2f6969a5b1)'>
          <path
            fill='currentColor'
            fillRule='evenodd'
            d='M14.756 10.164c1.665-.962 1.665-3.366 0-4.329L6.251.918C4.585-.045 2.5 1.158 2.5 3.083v9.834c0 1.925 2.085 3.128 3.751 2.164z'
            clipRule='evenodd'
          />
        </g>
        <defs>
          <clipPath id='IconifyId18e1439b2f6969a5b1'>
            <path fill='currentColor' d='M0 0h16v16H0z' />
          </clipPath>
        </defs>
      </g>
    </svg>
  )
}

export const PauseAudio = (props) => {
  return (
    <svg
      {...props}
      xmlns='http://www.w3.org/2000/svg'
      width='1.5em'
      height='1.5em'
      viewBox='0 0 16 16'
    >
      <path
        fill='currentColor'
        fillRule='evenodd'
        d='M1.5 3.5a2 2 0 0 1 2-2H5a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H3.5a2 2 0 0 1-2-2zm7.5 0a2 2 0 0 1 2-2h1.5a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H11a2 2 0 0 1-2-2z'
        clipRule='evenodd'
      />
    </svg>
  )
}

export const LiveAudio = (props) => {
  return (
    <svg
      {...props}
      xmlns='http://www.w3.org/2000/svg'
      width='1.5em'
      height='1.5em'
      viewBox='0 0 24 24'
    >
      <path
        fill='currentColor'
        d='M6.343 4.938a1 1 0 0 1 0 1.415a8.003 8.003 0 0 0 0 11.317a1 1 0 1 1-1.414 1.414c-3.907-3.906-3.907-10.24 0-14.146a1 1 0 0 1 1.414 0m12.732 0c3.906 3.907 3.906 10.24 0 14.146a1 1 0 0 1-1.415-1.414a8.003 8.003 0 0 0 0-11.317a1 1 0 0 1 1.415-1.415M9.31 7.812a1 1 0 0 1 0 1.414a3.92 3.92 0 0 0 0 5.544a1 1 0 1 1-1.415 1.414a5.92 5.92 0 0 1 0-8.372a1 1 0 0 1 1.415 0m6.958 0a5.92 5.92 0 0 1 0 8.372a1 1 0 0 1-1.414-1.414a3.92 3.92 0 0 0 0-5.544a1 1 0 0 1 1.414-1.414m-4.186 2.77a1.5 1.5 0 1 1 0 3a1.5 1.5 0 0 1 0-3'
      />
    </svg>
  )
}

export const Arrow = (props) => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='3rem'
      height='3rem'
      viewBox='0 0 24 24'
      {...props}
    >
      <path
        fill='currentColor'
        d='m9.55 12l7.35 7.35q.375.375.363.875t-.388.875q-.375.375-.875.375t-.875-.375l-7.7-7.675q-.3-.3-.45-.675t-.15-.75q0-.375.15-.75t.45-.675l7.7-7.7q.375-.375.888-.363t.887.388q.375.375.375.875t-.375.875z'
      />
    </svg>
  )
}

export const Hashtag = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='3rem'
      height='3rem'
      viewBox='0 0 24 24'
    >
      <path
        fill='currentColor'
        d='M24.01 6.797a1.341 1.341 0 0 0-1.034-.487h-.035h.002h-3.441l.954-4.643A1.4 1.4 0 0 0 20.188.48l.002.003a1.345 1.345 0 0 0-1.036-.486h-.033h.002c-.9.029-1.643.665-1.835 1.51l-.003.013l-.259 1.262c-.008.032-.022.062-.029.094l-.708 3.435h-5.793l.954-4.643a1.4 1.4 0 0 0-.268-1.187l.002.003a1.345 1.345 0 0 0-1.036-.486h-.033h.002c-.9.029-1.643.665-1.835 1.51l-.003.013l-.27 1.313c0 .015-.01.028-.013.042l-.702 3.435H3.547c-.9.029-1.643.664-1.836 1.509l-.003.013a1.4 1.4 0 0 0 .267 1.188l-.002-.003c.248.298.619.487 1.035.487h.034h-.002h3.597l-1.022 4.97H1.864c-.9.029-1.643.665-1.836 1.51l-.003.013a1.404 1.404 0 0 0 .267 1.185L.29 17.18c.249.298.62.487 1.036.487h.034h-.002h3.597l-.96 4.668a1.4 1.4 0 0 0 .268 1.187l-.002-.003c.249.298.62.487 1.036.487h.033h-.002a1.947 1.947 0 0 0 1.836-1.51l.003-.013l.99-4.814h5.796l-.96 4.668a1.4 1.4 0 0 0 .268 1.187l-.002-.003c.249.298.62.486 1.036.486h.033h-.002a1.945 1.945 0 0 0 1.835-1.51l.003-.013l.99-4.814h3.6a1.947 1.947 0 0 0 1.836-1.509l.003-.013a1.388 1.388 0 0 0-.263-1.188l.002.003a1.348 1.348 0 0 0-1.037-.487h-.036h.002h-3.447l1.022-4.97h3.6a1.947 1.947 0 0 0 1.836-1.51l.003-.013a1.401 1.401 0 0 0-.268-1.188l.002.003zM8.819 14.472l1.022-4.97h5.796l-1.022 4.97z'
      />
    </svg>
  )
}

export const Museum = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='3rem'
      height='3rem'
      viewBox='0 0 15 15'
    >
      <path
        fill='currentColor'
        d='M7.5 0L1 3.5V4h13v-.5zM2 5v5l-1 1.6V13h13v-1.4L13 10V5zm2 1h1v5.5H4zm3 0h1v5.5H7zm3 0h1v5.5h-1z'
      />
    </svg>
  )
}

export const Community = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='3rem'
      height='3rem'
      viewBox='0 0 16 16'
    >
      <path
        fill='currentColor'
        d='M3 3a2 2 0 1 1 4 0a2 2 0 0 1-4 0m6.779 1.584l.042.032a2 2 0 1 0-.042-.032M6.268 6A2 2 0 1 1 9.73 7.998A2 2 0 0 1 6.268 6M2.5 6h2.67a3.013 3.013 0 0 0 .594 3H5.5a2.501 2.501 0 0 0-2.355 1.658a3.733 3.733 0 0 1-.933-.543C1.46 9.51 1 8.616 1 7.5A1.5 1.5 0 0 1 2.5 6m8 3c1.085 0 2.009.691 2.355 1.658c.34-.139.654-.32.933-.543C14.54 9.51 15 8.616 15 7.5A1.5 1.5 0 0 0 13.5 6h-2.67c.11.313.17.65.17 1a2.99 2.99 0 0 1-.764 2zm1.387 1.928c.073.176.113.37.113.572c0 1.116-.459 2.01-1.212 2.615C10.047 14.71 9.053 15 8 15c-1.053 0-2.047-.29-2.788-.885C4.46 13.51 4 12.616 4 11.5A1.496 1.496 0 0 1 5.5 10h5a1.5 1.5 0 0 1 1.387.928'
      />
    </svg>
  )
}

export const Music = () => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='3rem'
      height='3rem'
      viewBox='0 0 24 24'
    >
      <path
        fill='currentColor'
        fillRule='evenodd'
        d='M22 7.019v-4a1.008 1.008 0 0 0-.354-.782a.998.998 0 0 0-.829-.22L8.852 4.01A1.002 1.002 0 0 0 8 5.017v9.519A4 4 0 1 0 10 18V9.846L20 8.18v4.355A4 4 0 1 0 22 16z'
        clipRule='evenodd'
      />
    </svg>
  )
}
